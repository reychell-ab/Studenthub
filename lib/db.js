...
// ── PLAN DE CARRERA ───────────────────────────────────────
export async function guardarPlanCarrera(uid, plan) {
  const ref = doc(db, "usuarios", uid, "planCarrera", "actual");
  const snap = await getDocs(collection(db, "usuarios", uid, "planCarrera"));
  if (snap.empty) {
    return addDoc(collection(db, "usuarios", uid, "planCarrera"), { ...plan, actualizadoEn: serverTimestamp() });
  }
  return updateDoc(doc(db, "usuarios", uid, "planCarrera", snap.docs[0].id), { ...plan, actualizadoEn: serverTimestamp() });
}
...
export async function actualizarCursosPlan(uid, planId, ciclos) {
  return updateDoc(doc(db, "usuarios", uid, "planCarrera", planId), { ciclos, actualizadoEn: serverTimestamp() });
}
